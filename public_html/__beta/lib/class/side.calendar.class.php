...
class SideCalendar{
	...
	/* CONSTRUCTOR */
	function SideCalendar($date = NULL, $year = NULL, $month = NULL){
	...
	function output_calendar_better($year = NULL, $month = NULL, $calendar_class = 'calendar'){
		...
		//------------------------------------------- create first date of month
		$month_start_date = strtotime($year . "-" . $month . "-01");
		$mmm = date('F', strtotime($year . "-" . $month . "-01"));
		$prev_month = date('Y-m-d', strtotime('-1 month', $month_start_date));
		$prev_month1 = date('M', strtotime('-1 month', $month_start_date));
		$next_month = date('Y-m-d', strtotime('+1 month', $month_start_date));
		$next_month1 = date('M', strtotime('+1 month', $month_start_date));
		...
		//----------------------- calc offset to find number of cells to prepend
		$start_week_offset = $first_day_falls_on - $this->week_start;
		$prepend = ( $start_week_offset < 0 )? 7 - abs($start_week_offset) : $this->week_start;
		//-------------------------- last day of month falls on what day of week
		$last_day_falls_on = $this->day_of_week($month_end_date);
		...
		//get month and smaller buttons
		
		$output .= '
			<div id="theCalendar_top">
				<a href="' . URL . '/small_calendar/' . $prev_month . '" class="prev_month small_link" id="prev_month" name="' . $prev_month . '">' . $prev_month1  . '</a>
				<h1>' . $mmm . ' ' . $year . '</h1>
				<a href="' . URL . '/small_calendar/' . $next_month . '" class="next_month small_link" id="next_month" name="' . $next_month . '">' . $next_month1  . '</a>
			</div>';

		//------------------------------------------------- start table, caption
		$output  .= "<div class=\"" . $calendar_class . "\">\n";
		
		$col = '';
		$th = '';
		for( $i=1,$j=$this->week_start,$t=(3+$this->week_start)*86400; $i<=7; $i++,$j++,$t+=86400 ){
			$localized_day_name = gmstrftime('%A',$t);
			$th .= "\t<div class=\"day\" title=\"" . ucfirst($localized_day_name) ."\">" . strtoupper($localized_day_name{0})."</div>\n";
			$j = ( $j == 7 )? 0 : $j;
		}
		...
		for($i=$prepend;$i>=1;$i--){
				...
				$pad_date = date('Y-m-d', strtotime('-' . $i . ' days', $month_start_date));
				$ddd = date('d', strtotime($pad_date));
				$event = new Calendar($pad_date);
				$output .= $event->print_day($types, 'past');
				
				//$output .= '	<div class="day past_month"><div class="day_number" id="' . $pad_date . '_prev">' . $pad_date . '</div></div> 					';
		...
		for($day=1,$cell=$prepend+1; $day<=$days_in_month; $day++,$cell++){
			...
			*/
			...
			$day = str_pad($day, 2, '0', STR_PAD_LEFT);
			$day_date = $year . "-" . $month . "-" . $day;
			$dow = date('D', strtotime($day_date));
			
			
			
				$event = new Calendar($day_date);
				$output .= $event->print_day($types);
		...
		if( $cell > 1 ){
			$dd = 1;
			for($i=$cell;$i<=7;$i++){
					$ddd  = date('d', strtotime('+' . $dd . ' days', $month_end_date));
					$pad_date = date('Y-m-d', strtotime('+' . $dd . ' days', $month_end_date));
					$event = new Calendar($pad_date);
					$output .= $event->print_day($types, 'future');
					
					//$output .= $event->print_day('future');
					//$output .= "\t<div class=\"day future_month\">$pad_date</div>\n";
				$dd++;
			}
			$output .= "</div>\n";
		}
