	...
	class User {
		...
		function User($var="", $type="") {
			if (is_numeric($var) && ($var) && $type=="") {
				...
				if($this->passes_in_tests($var['last_name']) && $this->passes_in_tests($var['first_name']) && $this->passes_in_tests($var['phone_number']) && $this->is_valid_email($email) && $this->passes_in_tests($email)) {
					$this->email = $email;
					$this->check_email();
		...
		function send_message($msg) {
			...
			if(!$this->passes_in_tests($msg))
				...
				$f .= 'Volunteer? ' . ($this->add2 == 'yes') ? 'Yes' : 'No';
				$f .= 'Comments:' . PHP_EOL;
