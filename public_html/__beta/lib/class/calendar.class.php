<?
	class Calendar {
		...
		function Calendar($var="") {
			...
			} else if(isset($var) && $var != '' && !is_array($var)) {
				require_once(CONNECTION);
				if($var != $this->clean_input($var)) {
					exit('error');
				} else {
					$sql = "SELECT * FROM calendar WHERE date = '$var'";
				   $result = @$mysqli->query($sql);
				   ...
				   if($count > 0) {
						...
						if($var == date('Y-m-d', strtotime($var))) {
							$this->date = $var;
							$this->calID = 0;
						}
					}
  ...
  /*      
     ...
     */
        ...
        function has_event() {
			...
			return ($this->calID == 0) ? false : true;
		...
		function latest_events() {
			$t = date('Y-m-d');
			$sql = "SELECT * FROM calendar WHERE date > '$t' ORDER BY date ASC LIMIT 15";
			$result = @$mysqli->query($sql)   or die('Query p list failed: ' . $mysqli->error);
			$count=@$result->num_rows;
			if($count == 0) {
				...
				while($row = @$result->fetch_assoc()) {
					$dd = date('F d, Y', strtotime($row['date']));
					echo '<p><span>' . $dd . '</span><br/>';
					echo $row['title'];
					echo '</p>';
				}
	...
	/*	
		function print_link() {
			...
			$ret .= '<a href="' . URL . '/calendar.html?date=' . $this->date . '" class="event_day" id="' . $this->date . '">' . date('d', strtotime($this->date)) . '</a>';
		...
		function print_day() {
			$ret = '';
			$ret .= '<div class="day" id="' . $this->date . '_day">';
			if($this->calID != 0)
				$ret .= $this->print_link();
			else 
				$ret .= date('d', strtotime($this->date));
			$ret .= '</div>';
	...
	*/
		...
		function print_link($type) {
			$ret = '';
			
			$ret .= '<a href="' . URL . '/calendar.html?date=' . $this->date . '" class="event_day ' . $type . '" id="' . $this->date . '">' . date('d', strtotime($this->date)) . '</a>';
			
			return $ret;
		}
		
		function print_day($types, $tense = 'present') {
			$ret = '';
			$ret .= '<div class="day '. $types[$this->type] . ' ' . $tense . '" id="' . $this->date . '_day">';
			if($this->calID != 0)
				$ret .= $this->print_link($types[$this->type]);
			else 
				$ret .= date('d', strtotime($this->date));
			$ret .= '</div>';
		...
		function print_pop_day() {
			$types = $this->get_formatted_types();
			$out = '';
			$dd = date('F d, Y', strtotime($this->date));
				$out .= '<div id="theDay_inside">
					<div class="cal_detail ' . $types[$this->type] . '_outline">
					<h2 class="cal_head ' . $types[$this->type] . '">' . $dd . '</h2>
